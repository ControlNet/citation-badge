...
citation_metadata = {
    ...
    "web_of_science": {"status": "not_attempted", "peer_reviews": 0, "error": None},
}
# -----------------------------------
...
try:
    ...
    citation_metadata["google_scholar"]["5y_citations"] = author.get("citedby5y", 0)
    ...
    citation_metadata["google_scholar"]["total_i10index"] = author.get("i10index", 0)
    citation_metadata["google_scholar"]["5y_i10index"] = author.get("i10index5y", 0)
    ...
    with open(os.path.join("dist", "all.svg"), "wb") as f:
        f.write(
            requests.get(
                f"https://img.shields.io/badge/citations-{total_cite}-_.svg?color=3388ee&style=flat-square"
            ).content
        )
    ...
    for pub in author["publications"]:
        ...
        with open(os.path.join("dist", f"{pub_id}.svg"), "wb") as f:
            f.write(
                requests.get(
                    f"https://img.shields.io/badge/citations-{pub_cite}-_.svg?color=3388ee&style=flat-square"
                ).content
            )
...
try:
    ...
    if citation_metadata["google_scholar"]["status"] == "success":
        ...
        if previous_data.get("google_scholar", {}).get("status") == "success":
            ...
            print(
                "Preserving previous Google Scholar data due to current failure",
                flush=True,
            )
        else:
            final_data["google_scholar"] = citation_metadata["google_scholar"]
    ...
    if citation_metadata["web_of_science"]["status"] == "success":
        ...
        if previous_data.get("web_of_science", {}).get("status") == "success":
            final_data["web_of_science"] = previous_data["web_of_science"]
            print(
                "Preserving previous Web of Science data due to current failure",
                flush=True,
            )
        else:
            final_data["web_of_science"] = citation_metadata["web_of_science"]
    ...
    if should_update or not os.path.exists(citation_json_path):
        ...
        print(
            "No successful updates and previous data exists - preserving existing citation.json",
            flush=True,
        )
...
"""
    ...
    if args.wos or wos_overwrite_raw is not None:
        ...
        summary_content += (
            f"| Web of Science  | ⚠️ Skipped | {wos_status['reason']:<32} |\n"
        )
